...
def startup():
    with db() as c:
        ...
        c.execute("""CREATE UNIQUE INDEX IF NOT EXISTS uq_task_claim_once
          ...
          WHERE task_id <> 'daily'
        """)
        c.execute("""CREATE INDEX IF NOT EXISTS idx_task_claims_daily
...
def tasks(user=Depends(current_user)):
    with db() as c:
        rows = c.execute(
            """
            SELECT t.id, t.title, t.description, t.amount,
              CASE
                WHEN t.id = 'daily' THEN EXISTS (
                  SELECT 1 FROM task_claims tc
                  WHERE tc.user_id = %s AND tc.task_id = 'daily'
                    AND tc.created_at >= date_trunc('day', now())
                )
                ELSE EXISTS (
                  SELECT 1 FROM task_claims tc
                  WHERE tc.user_id = %s AND tc.task_id = t.id
                )
              END AS claimed
            FROM tasks t
            WHERE t.active = true
